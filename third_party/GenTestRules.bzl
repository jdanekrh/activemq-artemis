...
# https://groups.google.com/forum/#!msg/bazel-discuss/MFYYkJk1tFs/UVqip2aBAQAJ
# https://gist.github.com/jart/9a9f48e216fd747275012d2f2364ac09
...
def GenTestRules(name,
  ...
  for test in _get_test_names(test_files):
    ...
    native.java_test(name = prefix + test,
                     runtime_deps = deps,
                     # because of the javaagent, bug in bazel https://github.com/bazelbuild/bazel/issues/1566
                     resources = resources,
                     size = test_size,
                     jvm_flags = jvm_flags,
                     args = args,
...
def _get_test_names(test_files):
  ...
  for test_file in test_files:
    if not test_file.endswith("Test.java"):  # TODO: surefire has different rules
      continue
